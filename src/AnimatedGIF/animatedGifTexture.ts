...
/**
 * Typings related to our Gif library as it does not include a d ts file.
 */
...
export class AnimatedGifTexture extends BaseTexture {
  ...
  public _frames: Nullable<GifFrame[]> = null;
  private _currentFrame: Nullable<GifFrame> | undefined;
  private _previousFrame: Nullable<GifFrame> | undefined;
  private _nextFrameIndex = 0;
  private _previousDate: number | undefined;

  private _patchEffectWrapper: EffectWrapper | undefined;
  private _patchEffectRenderer: EffectRenderer | undefined;
  private _renderLoopCallback: (() => void) | undefined;

  private _renderTarget: RenderTargetWrapper | undefined;
  ...
  private _loadGifTexture(): void {
    ...
    const callback: any = (buffer: ArrayBuffer) => {
  ...
  private _parseGifData(buffer: ArrayBuffer): void {
    ...
    (this._frames as any) = decompressFrames(gifData, true);
  }
  ...
  private _renderFrame(): void {
    ...
    if (
      this._currentFrame &&
      PrecisionDate.Now - this._previousDate! < this._currentFrame.delay
    ) {
      return;
    }
  ...
  private _drawPatch(): void {
    ...
    const frame: any = this._currentFrame;
    ...
    // THIS LINE IS CHANGED TO CHECK oldViewPort
    if (oldViewPort) {
